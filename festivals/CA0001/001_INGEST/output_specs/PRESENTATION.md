# Ingest presentation — CA0001 cans

User already directed: build the festival and implement. Specs below match that.

## What we are building

`veronica-agent/cans`. Type a line, **she** says it. `cans keep take.wav` pins anyone else. Go Charm booth. Clone sidecar if needed.

## Locked (do not re-ask)

| Decision | Value |
|----------|--------|
| Name | cans |
| Visibility | public |
| Default voice | Veronica kept ref, cloned |
| Other voices | keep a wav, not a menu |
| Language | Go TUI; Python only for clone |
| Not | Rust, Kokoro Bella, mic, Ollama |

## P0

`cans say`, `cans` booth, TTFA, keep, her identity on commits.

## Ask

These specs match the session. Proceeding to plan and implement as directed.
